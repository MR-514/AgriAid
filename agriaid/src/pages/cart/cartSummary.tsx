...
type CartSummaryProps = {
    productsInCart: any[]; // Replace with actual type if known
    itemsInCart: number;
    totalPrice: number;
    shippingAddressEntered: boolean;
    setShippingAddressEntered: (entered: boolean) => void;
    disabled: boolean;
};
...
const CartSummary: React.FC<CartSummaryProps> = ({
    ...
    setShippingAddressEntered,
    disabled
}) => {
    const errorTimer = useRef<number>(0);
    const positiveTimer = useRef<number>(0);
    ...
    const [positiveAlert, setPositiveAlert] = useState<boolean>(false);
    ...
    const finalAmount = (): number => itemsSubtotal() - promoCode;
    ...
    // Check coupon code
    const checkCoupon = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
